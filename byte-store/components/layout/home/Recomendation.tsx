...
const Recomendation = () => {
  return (
    <section
      id="recomendation"
      className="bg-black-one text-white max-w-[1360px] mx-auto w-full px-4 py-8"
    >
      <div className="flex flex-row justify-between items-center mb-6">
        <h2 className="text-xl font-semibold">Recommendation</h2>
        <div className="font-medium flex items-center gap-1 cursor-pointer hover:text-orange-one transition-colors text-sm sm:text-base">
      ...
      >
        {recomendationProducts.map((product, index) => (
          <SimpleProductCard key={index} {...product} />
        ))}
      </div>
    </section>
  );
